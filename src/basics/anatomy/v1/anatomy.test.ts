...
describe("@m0saic-starter/basics/anatomy/v1", () => {
  ...
  it("is deterministic — identical props, identical document", async () => {
    const a = await AnatomyV1.render({ text: "same" }, defaultCtx);
    const b = await AnatomyV1.render({ text: "same" }, defaultCtx);
    expect(a).toEqual(b);
  });

  it("fails fast on a malformed color", async () => {
    await expect(
      AnatomyV1.render({ backgroundColor: "blue" }, defaultCtx),
