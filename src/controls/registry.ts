...
export const controlsRegistry: StarterRegistryEntry[] = [
  ...
  },
  {
    ...
    title: "24 · Group Fields",
    description:
      "Related props that travel as one value: type group + fields nests ordinary prop definitions (same types, same controls — the accent is a normal colorPicker) under a single prop, edited as one fieldset and written in one edit, whole or not at all. Render guards the object as a unit: one guard for one idea. Drawn as the lower third this shape most often is.",
    tags: ["controls", "lesson"],
  },
  {
    slug: "range",
  ...
  },
  {
    ...
    title: "27 · Code Handoff",
    description:
      "A prop that flows the OTHER way: type code is a handoff — { language, code } shipped by the author in defaultProps, shown as a read-only, selectable, copyable code window (not an input), ignorable by render. For workflows where the user must run something outside the app; this lesson hands you the CLI command that renders itself headless.",
    tags: ["controls", "lesson"],
  },
  {
    slug: "m0-prop",
