 ...
 *      repoId "@m0saic-starter"; entryModule exists; every preview path
...
if (manifest) {
  ...
  if (manifest.repo?.repoId !== "@m0saic-starter") {
    fail(`manifest.repo.repoId is "${manifest.repo?.repoId}", must be "@m0saic-starter"`);
  }
...
if (mod) {
  ...
  if (!repo || typeof repo !== "object") {
    ...
    if (typeof repo.repoId !== "string" || repo.repoId !== "@m0saic-starter") {
      fail(`repo.repoId is ${JSON.stringify(repo.repoId)}, must be "@m0saic-starter"`);
    }
