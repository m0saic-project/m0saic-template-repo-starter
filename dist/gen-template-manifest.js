 ...
 */
const STARTER_ID_RE = /^@m0saic-starter\/([a-z0-9][a-z0-9-]*)\/([a-z0-9][a-z0-9-]*)\/v([1-9]\d*)$/;
/* ── Helpers ─────────────────────────────────────────────── */
...
function buildStarterManifest() {
    ...
    for (const chapter of template_registry_1.CHAPTERS) {
        ...
        for (const entry of chapter.entries) {
            ...
            assert(parsed, `Entry "${entry.templateId}" does not match "@m0saic-starter/<pack>/<slug>/vN"`);
    ...
    for (const pack of repo_1.TEMPLATE_PACKS) {
        const used = template_registry_1.templateRegistry.some((e) => e.templateId.startsWith(`@m0saic-starter/${pack.id}/`));
